            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                            ListElement { version: "v1.7.48"; title: "Auto-Detect Monitor Name on Fresh Install"; desc: "settings.json now uses real monitor name from sysfs instead of hardcoded eDP-1"; icon: "󰒃"; clr: "blue"; detail: "ONE-TIME MIGRATION: Existing users who had settings.json broken by v1.7.45/46 bug get it auto-fixed with correct monitor name. User's current settings backed up first. Creates a migration flag so this NEVER happens again. ADDED: Auto-detect monitor name on fresh install — reads first connected display from sysfs, injects real name into settings.json instead of hardcoded eDP-1. Falls back to default if detection fails or jq is missing. Final validation checks JSON is valid before deploying." }
                            ListElement { version: "v1.7.47"; title: "CRITICAL: settings.json Never Overwritten on Update"; desc: "Fixed: every update wiped user's monitor layout, keybinds, and startup apps"; icon: "󰒃"; clr: "red"; detail: "CRITICAL FIX: install.sh line 247 force-overwrote ~/.config/hypr/settings.json on every update — violating the NEVER TOUCH rule. Users with custom monitor setups (multiple displays, different resolutions, custom scaling) lost their configuration every time they updated. CHANGED: settings.json now only deployed on fresh install if the file doesn't exist. Existing settings.json is never touched." }
