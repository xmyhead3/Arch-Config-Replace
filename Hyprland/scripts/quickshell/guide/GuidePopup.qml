            ...
            Item {
                ...
                ColumnLayout {
                    ...
                    ListView {
                        ...
                        model: ListModel {
                            ListElement { version: "v1.7.17"; title: "Fix Battery Sound in Quickshell"; desc: "battery_fetch.sh now uses pw-play/paplay so audio always plays in Quickshell Process"; icon: "󰁹"; clr: "green"; detail: "Low battery notifications (20%/10%/5%/3%) now play audio reliably for all users. The _play_bat_sound() function previously only tried mpg123 and ffplay, which fail silently when spawned from Quickshell's Process sandbox (missing PipeWire/PulseAudio env vars). Added pw-play and paplay as the primary audio players — both inherit the correct audio session from PipeWire. All users already have pipewire-pulse installed, so this fix is zero-dependency." }
